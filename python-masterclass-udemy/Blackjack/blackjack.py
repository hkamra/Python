...
def deal_player():
    ...
    # # whenever we use a variable in function having the same name as global
    # # python creates a local variable with the same name and use it for further reference as declared below
    # # python will only use global variable if we don't modify it in the function
    # # player_score = 0
    # global player_score  # this will avoid the shadowing error
    # global player_ace
...
# below declarations are only required if we are using the lower definition of function
# deal_player
# player_score = 0
# player_ace = False
